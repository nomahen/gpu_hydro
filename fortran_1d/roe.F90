...
#include "definition.h"  
  ...
  do kWaveNum = 1, NUMB_WAVE
     vec(DENS_VAR:ENER_VAR) = dot_product(leig(DENS_VAR:ENER_VAR,kWaveNum),&
                                            uR(DENS_VAR:ENER_VAR) &
                                           -uL(DENS_VAR:ENER_VAR)&
                                          )*abs(lambda(kWaveNum))*reig(DENS_VAR:ENER_VAR,kWaveNum)
     ! summation
